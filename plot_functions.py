...
def plot_importants(State: "Network_State", Variabs: "User_Variables", desired: np.ndarray, M: NDArray[np.int_]) -> None:
    ...
    if Variabs.Nin==1 and Variabs.Nout==2:  # Allostery
        A = M[0]  # A = x_hat/p_in
        B: float = M[1]  # B = y_hat/p_in
        Rl_subs: float = 1.0
        R_theor = State.input_drawn_in_t[0]*np.array([(1-A)/(A*(1+1/Rl_subs)-B/Rl_subs), (1-B)/(B*(1+1/Rl_subs)-A/Rl_subs)])
...
def PlotNetwork(p: np.ndarray, u: np.ndarray, K: np.ndarray, BigClass: "Big_Class", mode: str='u_p', nodes: bool=True, edges: bool=True, savefig: bool=False) -> None:
    """
    ...
    if edges:
        if mode=='u_p':
            ...
            nx.draw_networkx_edges(NET, pos_lattice, edgelist=r_edges_positive, edge_color='k', arrows=True, width=edgewidths_k_positive,
                                   arrowstyle='->')
            nx.draw_networkx_edges(NET, pos_lattice, edgelist=r_edges_negative, edge_color='k', arrows=True, width=edgewidths_k_negative,
                                   arrowstyle='<-')
        elif mode=='R_p':
